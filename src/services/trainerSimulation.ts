...
const PROTECTIVE_RESPONSE_PATTERNS = [
  ...
  /(?:^|[^а-яё])(?:только\s+(?:демо|прототип|часть|базов)|mvp|прототип|минимальн|поэтапн|чернов(?:ик|ой))/i,
]

const IMPOSED_DEADLINE_PATTERN =
  /(?:^|[^а-яё])(?:к|до)\s+(?:четверг|пятниц)|(?:^|[^а-яё])в\s+четверг(?:\s+вечером)?|(?:^|[^а-яё])за\s+(?:два|три|2|3)\s+дн/i

const DELIVERY_COMMITMENT_PATTERNS = [
  /(?:^|[^а-яё])(?:сделаем|успеем|выполним|сдадим|подготовим|реализуем|обучим|переделаем)(?=$|[^а-яё])/i,
  /(?:^|[^а-яё])(?:помощник|ассистент|дизайн|главн(?:ая|ую)|результат|вс[её])[^.!?]{0,80}(?:будет|будут)\s+(?:готов|готовы|сделан|сделаны|обучен|обучены|реализован|реализованы|заверш[её]н|премиальн)/i,
]

/**
 * Detects clear or implicit capitulation before asking the NPC for a response.
 * Protective language wins so phrases such as «Да, понимаю, но ночью работать
 * не будем» and limited MVP proposals are not mistaken for unconditional agreement.
 */
export function isDisadvantageousAgreement(text: string): boolean {
  ...
  const explicitCapitulation = CAPITULATION_PATTERNS.some((pattern) =>
    pattern.test(normalized),
  )
  const acceptsImposedDeadline =
    IMPOSED_DEADLINE_PATTERN.test(normalized) &&
...
- Обещание в будущем времени тоже является капитуляцией, даже без слов «согласен» или «хорошо». Пример: «в четверг помощник будет обучен, а дизайн будет премиальным» — пользователь принял и весь объём, и навязанный срок.
- Отличай это от ограниченной альтернативы: «к пятнице покажем только прототип/MVP, остальное позже» защищает команду и может быть Win-Win.
...
- Обещание «к четвергу/пятнице всё будет готово» — такая же уступка, даже если пользователь не произносит «я согласен». Ответы «Отлично, договорились» и «это именно то, что нужно» в этой ветке запрещены.
